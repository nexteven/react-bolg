(function (name, factory) {
    ...
    let mouseInfo = function (e) {
        ...
        let mousex = e ? Math.max(0, e.pageX || e.clientX || 0) : 0;
        let mousey = e ? Math.max(0, e.pageY || e.clientY || 0) : 0;
        return {
            mousex: mousex,
            mousey: mousey,
    ...
    Factory.prototype = {
        ...
        init: function () {
            try {
                ...
                this._canvas.style['background-color'] = 'rgb(35,35,35,0.1)';
        ...
        addRibbon: function () {
            ...
            while (true) {
                if (stop <= 0) break;
                stop--;
                movex = Math.round((Number(Math.random()) - 0.2) * this._options.horizontalSpeed);
                movey = Math.round((Number(Math.random()) - 0.5) * (this._height * 0.25));
                point3 = new Point();
                point3.copy(point2);
                if (dir === 'right') {
        ...
        _drawRibbonSection: function (section) {
            if (section) {
                ...
                if (section.delay <= 0) {
                    section.phase += 0.02;
                    section.alpha = Number(Math.sin(section.phase));
                ...
                let l = this._options.colorBrightness;
                let c = 'hsla(' + section.color + ', ' + s + ', ' + l + ', ' + section.alpha + ' )';
                this._context.save();
